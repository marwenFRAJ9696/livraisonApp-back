...
public class PackageServiceImpl implements PackageService {
    ...
    Collection<PackageResponse> mapPakcageToResponse(Collection<Package> packages) {
        ...
        packages.stream().forEach(p -> {
            ...
            packageResponse.isEchange(p.isEchange());
            packageResponse.isFragile(p.isFragile());
            packageResponse.isMoyen(p.isMoyen());
            packageResponse.isPetit(p.isPetit());
            packageResponse.isGrand(p.isGrand());
            packageResponse.isExtraLarge(p.isExtraLarge());
    ...
    public Collection<PackageResponse> getAllPackageByCreatedDate() {
        ...
        List<Package> sortedColis = colis.stream()
                .sorted(Comparator.comparing(Package::getCreatedDate).reversed())
                .collect(Collectors.toList());
        Collection<PackageResponse> packageResponses = mapPakcageToResponse(sortedColis);
    ...
    public PackageResponse getPackageById(Integer id) {
        ...
        if (optionalPackage.isPresent()) {
            ...
            addPackage(packageResponse, p);
        }
        return packageResponse;
    }

    private void addPackage(PackageResponse packageResponse, Package p) {
        packageResponse.setCity(p.getCity());
        packageResponse.setPackageNumber(p.getPackageNumber());
        packageResponse.setArticleNumber(p.getArticleNumber());
        packageResponse.setComment(p.getComment());
    ...
    @Override
    public PackageResponse updatePackageById(Integer id, PackageRequest request) {
        Optional<Package> optionalPackage = packageRepository.findById(id);
        PackageResponse packageResponse = new PackageResponse();
        if (optionalPackage.isPresent()) {
            Package p = optionalPackage.get();
            p.setFullName(request.getFullName());
            p.setGovernorate(request.getGovernorate());
            p.setCity(request.getCity());
            p.setTelNumber(request.getTelNumber());
            p.setTelNumber2(request.getTelNumber2());
            p.setDesignation(request.getDesignation());
            p.setFullAddress(request.getFullAddress());
            p.setArticleNumber(request.getArticleNumber());
            p.setPackageCanBeOpen(request.getIsPackageCanBeOpen() != null && (request.getIsPackageCanBeOpen().equals("OUI") ? true : false));
            p.setPackageNumber(request.getPackageNumber());
            p.setDeliveryType(Enum.valueOf(DeliveryType.class, request.getDeliveryType()));
            p.setPaymentMode(Enum.valueOf(PaymentMode.class, request.getPaymentMode()));
            p.setPriceNet(request.getPriceNet());
            p.setComment(request.getComment());
            p.setEchange(request.getIsEchange().equals("true"));
            p.setFragile(request.getIsFragile().equals("true"));
            p.setPetit(request.getIsPetit().equals("true"));
            p.setMoyen(request.getIsMoyen().equals("true"));
            p.setGrand(request.getIsGrand().equals("true"));
            p.setExtraLarge(request.getIsExtraLarge().equals("true"));
